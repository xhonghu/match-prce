...
class Transformer(nn.Module):
    ...
    def _reset_parameters(self):
        '''
        该方法通过遍历模型的所有参数,并对维度大于1的参数进行 Xavier均匀初始化。
        '''
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        ...
        entity_embed = entity_embed.unsqueeze(1).repeat(1, bs, 1)   # [100,bs,256]
        triplet_embed = triplet_embed.unsqueeze(1).repeat(1, bs, 1)  # [200,bs,256]
        entity = entity.unsqueeze(1).repeat(1, bs, 1)  # [100,1,256]
        triplet = triplet.unsqueeze(1).repeat(1, bs, 1)  # [200,1,512]
        mask = mask.flatten(1) # [1,864]
        #经过编码器之后的特征
        ...
        return hs.transpose(1, 2), hs_t.transpose(1, 2), so_masks
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, entity, triplet, memory,
                ...
                pos: Optional[Tensor] = None,
                entity_pos: Optional[Tensor] = None,
                triplet_pos: Optional[Tensor] = None,
        ...
        for layer in self.layers:
            output_entity, output_triplet, sub_maps, obj_maps = layer(output_entity, output_triplet, entity_pos, triplet_pos, so_pos,  memory,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos, memory,
                memory_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None):

        # entity layer
        q_entity = k_entity = self.with_pos_embed(tgt_entity, entity_pos)
        tgt2_entity = self.self_attn_entity(q_entity, k_entity, value=tgt_entity)[0]
        #残差连接
...
def _get_clones(module, N):
    '''
    这个函数的目的是为了方便创建多个相同的模块实例，可以通过指定 N 来复制多个模块，并将它们存储在一个模块列表中供后续使用。
    '''
